...
npm run test:coverage       # v8 coverage (components incl. .ts hooks, lib, tokens, scripts); fails under the thresholds in vitest.config.ts — CI runs this (STU-889)
...
`scripts/build-manifest.mjs` runs as the last `postbuild` step and writes `dist/manifest.json` + `dist/migrations.json` (the two build outputs read by the Manfred DS CLI), exported as `./manifest.json` and `./migrations.json`, and copies both schemas to `dist/` (exported as `./manifest.schema.json` and `./migrations.schema.json`). It reads every non-test, non-story `.tsx` file one level deep, `src/components/<Dir>/*.tsx` (not `**`), with `react-docgen-typescript`, cross-references the barrel (`src/index.ts`) to keep only exported, PascalCase components, and parses `src/tokens/tokens.css` into primitive/semantic/contract layers by its `LAYER 2`/`LAYER 3` section headers. Both outputs are schema-validated against `scripts/manifest.schema.json` and `scripts/migrations.schema.json` (Ajv) before anything is written — an invalid manifest or an out-of-order/malformed `migrations.json` fails the build. `NON_COMPONENT_EXPORTS` is the allowlist for PascalCase value exports that docgen genuinely can't see (e.g. a string constant, not a component); every entry needs a one-line reason comment, and a missing name should first be treated as a suspected bug in `componentFiles`, not reflexively allowlisted. The prop filter drops the DOM attribute surface (`@types/react`, TypeScript `lib.*.d.ts`, recharts' DOM adapter) and sets each component's `extendsDom` when it dropped any. An exported component with zero props fails the build unless it is in `HTML_WRAPPER_COMPONENTS` (pure `React.HTMLAttributes` wrappers, one reason per entry); when docgen loses a real type (NavItem's union), add hand-written props to `PROP_OVERRIDES` instead. A test checks each override name is still declared in its source file.
...
Release checklist addition: if consumers must act (peer bump, removed API, setup change), add a `migrations.json` entry in the same PR.
